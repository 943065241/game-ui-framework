...
def build_parser() -> argparse.ArgumentParser:
    ...
    gated_plan_cmd = sub.add_parser("run-export-plan", help="Evaluate and persist the production Export Gate without mutating Project truth"); gated_plan_cmd.add_argument("task_id"); gated_plan_cmd.add_argument("--project", required=True); gated_plan_cmd.add_argument("--target")
    gated_execute_cmd = sub.add_parser("run-export-execute", help="Materialize reviewed active production Artifacts and create an Engine export"); gated_execute_cmd.add_argument("task_id"); gated_execute_cmd.add_argument("--project", required=True); gated_execute_cmd.add_argument("--target"); gated_execute_cmd.add_argument("--actor", required=True)
    gated_list_cmd = sub.add_parser("run-export-list", help="List gated Export records for a Task"); gated_list_cmd.add_argument("task_id"); gated_list_cmd.add_argument("--project", required=True)
    gated_show_cmd = sub.add_parser("run-export-show", help="Show one gated Export record"); gated_show_cmd.add_argument("task_id"); gated_show_cmd.add_argument("export_id"); gated_show_cmd.add_argument("--project", required=True)
    gated_rollback_cmd = sub.add_parser("run-export-rollback", help="Rollback Project truth mutations from one completed gated Export"); gated_rollback_cmd.add_argument("task_id"); gated_rollback_cmd.add_argument("export_id"); gated_rollback_cmd.add_argument("--project", required=True); gated_rollback_cmd.add_argument("--actor", required=True); gated_rollback_cmd.add_argument("--reason", required=True); gated_rollback_cmd.add_argument("--force", action="store_true")
    ...
    export_cmd = sub.add_parser("export", help="Validate and export existing Project Resource files deterministically"); export_cmd.add_argument("project"); export_cmd.add_argument("--target", default="generic"); export_cmd.add_argument("--output", type=Path); export_cmd.add_argument("--clean", action=argparse.BooleanOptionalAction, default=True)
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if args.command == "run-export-plan": print(json.dumps(Runtime(workspace).prepare_gated_export(args.project, args.task_id, target_engine=args.target), ensure_ascii=False, indent=2)); return 0
        if args.command == "run-export-execute": print(json.dumps(Runtime(workspace).execute_gated_export(args.project, args.task_id, target_engine=args.target, actor=args.actor), ensure_ascii=False, indent=2)); return 0
        if args.command == "run-export-list": print(json.dumps(Runtime(workspace).list_gated_exports(args.project, args.task_id), ensure_ascii=False, indent=2)); return 0
        if args.command == "run-export-show": print(json.dumps(Runtime(workspace).get_gated_export(args.project, args.task_id, args.export_id), ensure_ascii=False, indent=2)); return 0
        if args.command == "run-export-rollback": print(json.dumps(Runtime(workspace).rollback_gated_export(args.project, args.task_id, args.export_id, actor=args.actor, reason=args.reason, force=args.force), ensure_ascii=False, indent=2)); return 0
