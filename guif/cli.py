...
def build_parser() -> argparse.ArgumentParser:
    ...
    export_cmd = sub.add_parser("export", help="Validate and export project assets deterministically")
    export_cmd.add_argument("project")
    export_cmd.add_argument("--target", default="generic")
    export_cmd.add_argument("--output", type=Path)
    export_cmd.add_argument("--clean", action=argparse.BooleanOptionalAction, default=True)
    ...
    composite_cmd.add_argument("--feather", type=float, default=0.0)
    composite_cmd.add_argument("--threshold", type=int, default=1)
    composite_cmd.add_argument("--verify", action=argparse.BooleanOptionalAction, default=True)
    return parser
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        if args.command == "init":
            print(init_project(workspace, args.project)); return 0
        if args.command == "inspect":
            if args.project:
                ...
                payload = {
                    ...
                    "workflows": list_workflows(workspace),
                }
            print(json.dumps(payload, ensure_ascii=False, indent=2)); return 0
        if args.command == "plan":
            print(create_plan(workspace, args.project, args.requirement)); return 0
        ...
        if args.command == "record":
            print(record_memory(workspace, args.project, args.memory_type, args.message)); return 0
        if args.command == "theme-create":
            print(create_theme(workspace, args.project, args.name, args.description)); return 0
        ...
        if args.command == "workflow-list":
            print(json.dumps(list_workflows(workspace, args.project), ensure_ascii=False, indent=2)); return 0
        if args.command == "workflow-show":
            print(json.dumps(load_workflow(workspace, args.project, args.workflow_id).to_dict(), ensure_ascii=False, indent=2)); return 0
        ...
        if args.command == "resource-create":
            print(create_resource_manifest(workspace, args.project, args.resource_id, args.resource_type, args.width, args.height, args.file_format, alpha_required=args.alpha, target_engine=args.target_engine, output_name=args.output_name, source=args.source)); return 0
        ...
        if args.command == "resource-show":
            print(json.dumps(load_resource_manifest(args.path).to_dict(), ensure_ascii=False, indent=2)); return 0
        if args.command == "asset-validate":
            report = validate_asset_against_manifest(args.manifest, args.asset)
            print(json.dumps(report.to_dict(), ensure_ascii=False, indent=2)); return 0 if report.passed else 1
        if args.command == "export":
            report = export_project_assets(workspace, args.project, target_engine=args.target, output_dir=args.output, clean=args.clean)
            print(json.dumps(report.to_dict(), ensure_ascii=False, indent=2)); return 0 if report.passed else 1
        if args.command == "qa-pixels":
            report = compare_protected_pixels(args.original, args.edited, args.mask, args.tolerance)
            print(json.dumps(report.to_dict(), indent=2)); return 0 if report.passed else 1
        if args.command == "compose-edit":
            ...
            if args.verify:
                ...
                print(json.dumps(payload, indent=2)); return 0 if qa.passed else 1
