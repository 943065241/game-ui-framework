...
def test_project_schema_rejects_invalid_fields() -> None:
    ...
    assert any("PrivateThemeStore" in error for error in errors)
    assert any("created_at" in error for error in errors)


def test_validate_project_rejects_project_local_theme_binding(tmp_path: Path) -> None:
    root = init_project(tmp_path, "Demo")
    config_path = root / "project.json"
    config = json.loads(config_path.read_text(encoding="utf-8"))
    config["current_theme"] = "legacy-theme"
    config_path.write_text(json.dumps(config), encoding="utf-8")

    errors = validate_project(tmp_path, "Demo")

    assert "Project Theme content and bindings are private data; migrate them to PrivateThemeStore." in errors
