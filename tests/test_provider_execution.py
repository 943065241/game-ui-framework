...
def _create_ready_project(tmp_path: Path) -> None:
    ...
    Runtime(tmp_path).create_private_theme(
        ...
        {
            "description": "Synthetic abstract arcade UI direction for provider tests.",
...
def test_dry_run_executes_only_after_approval_and_registers_artifact(tmp_path: Path) -> None:
    ...
    with pytest.raises(ValueError, match="not ready"):
        runtime.execute_job(PROJECT, task.task_id, job_id)
    ...
    assert runtime.get_artifact(PROJECT, task.task_id, artifact["artifact_id"]) == artifact
    ...
    assert runtime.list_runs(PROJECT)[0]["artifact_count"] == 1
    assert runtime.list_runs(PROJECT)[0]["provider_execution_count"] == 1
...
def test_provider_capability_gate_rejects_before_execution(tmp_path: Path) -> None:
    ...
    with pytest.raises(ValueError, match="lacks required capabilities"):
        runtime.execute_job(PROJECT, task.task_id, job_id, provider_id="limited")

    persisted = runtime.load_task(PROJECT, task.task_id)
...
def test_provider_failure_is_persisted_without_losing_task_or_approval(tmp_path: Path) -> None:
    ...
    with pytest.raises(ProviderExecutionError, match="provider unavailable"):
        runtime.execute_job(PROJECT, task.task_id, job_id, provider_id="failing")
    ...
    assert runtime.list_artifacts(PROJECT, task.task_id) == ()
    assert (runtime.store.run_dir(PROJECT, task.task_id) / "executions.json").is_file()
